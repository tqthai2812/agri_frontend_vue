...
watch(
  ...
  (isOpen) => {
    ...
    if (props.editAddressId) {
      const address = props.addresses.find((item) => {
        return Number(item.id) === Number(props.editAddressId);
      });
    ...
    >
      ...
      >
        <div class="flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl">
          ...
          <div class="flex-1 overflow-y-auto px-5 py-5 sm:px-6">
            <template v-if="!showForm">
                ...
                >
                  ...
                  <div class="min-w-0 flex-1">
                    ...
                    <p class="mt-2 text-xs leading-5 text-slate-500">
                      {{ address.full_address || `${address.address_detail}, ${address.ward}, ${address.district}, ${address.province}` }}
                    </p>

                    <span class="mt-2 inline-flex rounded-full bg-slate-100 px-2.5 py-1 text-[10px] text-slate-500">
                      {{
              ...
              >
                <Icon
                  ...
                  class="mx-auto text-4xl text-[#07532b]"
                />
            ...
            >
              ...
              <div class="grid gap-4 sm:grid-cols-2">
                <label class="block">
                  <span class="address-label">Họ và tên</span>
              ...
              <div class="grid gap-4 sm:grid-cols-3">
                ...
                <label class="block">
                  <span class="address-label">Quận/Huyện</span>
                ...
                <label class="block">
                  <span class="address-label">Phường/Xã</span>

                  <input
                    v-model.trim="form.ward"
                    class="address-input"
                    placeholder="Hưng Lợi"
                  />
                </label>
              </div>

              <label class="block">
                <span class="address-label">Địa chỉ cụ thể</span>

                <textarea
                  v-model.trim="form.address_detail"
                  rows="3"
                  class="address-input resize-none"
          ...
          <footer
            v-if="!showForm"
            class="flex justify-end gap-3 border-t border-slate-100 px-5 py-4 sm:px-6"
          >
            <button
              type="button"
              class="rounded-full border border-slate-200 px-5 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-50"
