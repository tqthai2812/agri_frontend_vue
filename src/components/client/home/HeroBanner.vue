...
const FULL_TEXT = "Vật tư chính hãng – nền tảng của nền nông nghiệp xanh";
...
const heroStyle = {
    backgroundImage:
        "url('https://images.unsplash.com/photo-1500382017468-9049fed747ef?auto=format&fit=crop&w=2000&q=88')",
};

const displayedText = computed(() => {
    return characters.slice(0, currentIndex.value).join("");
});
...
function scheduleTyping(delay) {
    ...
    if (!isPaused.value) {
        typingTimer = window.setTimeout(runTypingEffect, delay);
    }
}
...
<template>
    <section
        ...
        :style="heroStyle" aria-labelledby="hero-title">
        ...
        <div class="mx-auto w-full max-w-[1440px] px-5 pb-12 pt-8 sm:px-8 lg:px-16">
            <h1 id="hero-title"
                ...
                <span aria-hidden="true" class="ml-1 inline-block h-[0.9em] w-[3px] translate-y-[0.08em] bg-[#ffd326]"
                    :class="isPaused ? 'opacity-50' : 'animate-pulse'"></span>
            </h1>
        </div>
        ...
        <button type="button"
            ...
            :aria-pressed="isPaused" :aria-label="isPaused ? 'Tiếp tục hiệu ứng đánh máy' : 'Dừng hiệu ứng đánh máy'"
            @click="toggleTypingEffect">
            <Icon :icon="isPaused ? 'mdi:play' : 'mdi:pause'" class="text-lg text-[#07532b]" />

            {{ isPaused ? "Tiếp tục hiệu ứng" : "Dừng hiệu ứng" }}
        </button>
