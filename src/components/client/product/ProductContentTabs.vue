...
const tabs = computed(() => [
    ...
    {
        id: "reviews",
        label: `Đánh giá (${props.product.review_count || props.reviews.length || 0})`,
...
const ratingDistribution = computed(() => {
    return [5, 4, 3, 2, 1].map((rating) => {
        const count = props.reviews.filter((review) => Number(review.rating) === rating).length;

        const percent = props.reviews.length
            ? Math.round((count / props.reviews.length) * 100)
            : 0;
                ...
                <aside>
                    <div class="rounded-2xl bg-[#fffaf0] p-6 text-center">
                        ...
                        <p class="mt-2 text-xs text-slate-400">
                            {{ product.review_count || reviews.length || 0 }} lượt đánh giá
                        </p>
                    </div>
