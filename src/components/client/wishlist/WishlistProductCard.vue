...
const productName = computed(() => {
    return product.value.product_name || product.value.name || "Sản phẩm";
});
...
const availablePackages = computed(() => {
    return (product.value.variants || [])
        ...
        .filter((pkg) => Number(pkg.quantity_available || 0) > 0);
});
...
const minPrice = computed(() => {
    ...
    const prices = availablePackages.value.map((pkg) => Number(pkg.price || 0));
...
const discountPercent = computed(() => {
    if (!oldPrice.value || oldPrice.value <= minPrice.value) {
        return 0;
    }

    return Math.round(((oldPrice.value - minPrice.value) / oldPrice.value) * 100);
});
...
const brandLabel = computed(() => {
    return (
        product.value.brand ||
        product.value.origin?.name ||
        product.value.origin?.origin_name ||
...
const categoryLabel = computed(() => {
    return (
        product.value.category?.name ||
        product.value.category?.category_name ||
        "Chưa phân loại"
    );
});
    ...
    <article v-if="viewMode === 'grid'"
        ...
        <label
            ...
            <input type="checkbox" class="size-4 cursor-pointer accent-[#07532b]" :checked="selected"
                :disabled="disabled" :aria-label="`Chọn ${productName}`" @change="emit('toggle-select', item.id)" />
        </label>

        <button type="button"
            class="absolute right-3 top-3 z-20 grid size-9 place-items-center rounded-full bg-white/95 text-red-500 shadow-md transition hover:scale-105 hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50"
            aria-label="Xóa khỏi danh sách yêu thích" :disabled="disabled" @click="emit('remove', item)">
        ...
        <RouterLink :to="{
            ...
            <img v-if="primaryImage" :src="primaryImage" :alt="productName" loading="lazy"
        ...
        <div class="flex flex-1 flex-col p-4">
            ...
            <div class="mt-3 flex items-center gap-2">
                <div class="flex" :aria-label="`${product.rating || product.average_rating || 0} trên 5 sao`">
                    <Icon v-for="star in 5" :key="star" icon="mdi:star" class="text-[14px]" :class="star <= Number(product.rating || product.average_rating || 0)
                        ? 'text-[#ffc400]'
                        : 'text-slate-200'" />
                </div>
            ...
            <div class="mt-auto pt-4">
                <button type="button"
                    class="flex h-10 w-full items-center justify-center gap-2 rounded-full text-xs font-bold transition disabled:cursor-not-allowed disabled:opacity-50"
                    :class="inStock
                        ? 'bg-[#07532b] text-white hover:bg-[#064522]'
                        : 'cursor-not-allowed bg-slate-100 text-slate-400'" :disabled="!inStock || disabled"
                    @click="emit('add-cart', item)">
                    <Icon :icon="availablePackages.length === 1
                        ? 'mdi:cart-plus'
                        : 'mdi:tune-variant'" class="text-lg" />
    ...
    <article v-else
        ...
        <div class="flex items-start gap-3 sm:gap-5">
            ...
            <RouterLink :to="{
                ...
                <img v-if="primaryImage" :src="primaryImage" :alt="productName"
            ...
            <div class="min-w-0 flex-1 md:flex md:items-center md:justify-between md:gap-7">
                <div class="min-w-0">
                    ...
                    <p class="mt-2 text-[10px] text-slate-400">
                        {{ categoryLabel }}
                        · {{ availablePackages.length }} quy cách còn hàng
                    </p>
                ...
                <div class="mt-4 shrink-0 md:mt-0 md:w-52 md:text-right">
                    ...
                    <div class="mt-3 flex gap-2 md:justify-end">
                        <button type="button"
                            class="grid size-10 shrink-0 place-items-center rounded-full border border-red-200 text-red-500 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50"
                            aria-label="Xóa khỏi danh sách yêu thích" :disabled="disabled"
                            @click="emit('remove', item)">
                            <Icon icon="mdi:trash-can-outline" class="text-lg" />
                        </button>

                        <button type="button"
                            class="flex h-10 flex-1 items-center justify-center gap-1.5 rounded-full px-4 text-[10px] font-bold transition disabled:cursor-not-allowed disabled:opacity-50 md:flex-none"
                            :class="inStock
                                ? 'bg-[#07532b] text-white hover:bg-[#064522]'
                                : 'cursor-not-allowed bg-slate-100 text-slate-400'" :disabled="!inStock || disabled"
                            @click="emit('add-cart', item)">
                            <Icon :icon="availablePackages.length === 1
                                ? 'mdi:cart-plus'
                                : 'mdi:tune-variant'" class="text-base" />
